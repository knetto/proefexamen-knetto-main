    ...
    @section('content')
        ...
        @else
            @foreach ($menucards as $menu)
                ...
                ?>
            @endforeach
        <h3 class="menukaartTitle">Dagmenu</h3>
            {{-- voorgerechten --}}
                                        ...
                                        @if ($dish->category_id == 1)
                                            @if (!in_array("$dish->id", $dishesId))
                                                <div class="adDishItem"> {{ $dish->name }}
                                                    <!-- Update Form -->
                                                    <form
                                                        action="{{ action('\App\Http\Controllers\dagmenuController@update', ['dagmenu' => $menu->id]) }}"
                                                        method="POST">
                                                        @csrf
                                                        @method('PUT')
...
@else
    ...
    @for ($i = 0; $i <= count($dishesId) - 1; $i++)
        @if ($dishes[$dishesId[$i] - 1]['category_id'] == 1)
            <!-- Update Form -->
            <form
            action="{{ action('\App\Http\Controllers\dagmenuController@update', ['dagmenu' => $menu->id]) }}"                method="POST">
                @csrf
                @method('PUT')
                <div class="form-group">
